...
describe("userFacingErrorMessage", () => {
  ...
  it("explains private order validation failures without refresh advice", () => {
    expect(
      userFacingErrorMessage(
        new Error("Request to /api/private/orders failed with HTTP 400")
      )
    ).toBe(
      "Private order was rejected by validation. Check available notes and order parameters, then retry."
    );
  });
