...
export function formatShortfallBps(
  side: "Buy" | "Sell",
  limitPrice: string,
  clearingPrice: string,
): string {
  const limit = Number(limitPrice);
  const clearing = Number(clearingPrice);
  if (!Number.isFinite(limit) || !Number.isFinite(clearing) || limit <= 0) return "—";
  const bps = side === "Buy"
    ? ((clearing - limit) / limit) * 10_000
    : ((limit - clearing) / limit) * 10_000;
  const formatted = Math.abs(bps) >= 100
...
export function implementationShortfallBpsValue(
  side: "Buy" | "Sell",
  ...
  const clearing = Number(clearingPrice);
  if (!Number.isFinite(reference) || !Number.isFinite(clearing) || reference <= 0) return null;
  return side === "Buy"
    ? ((clearing - reference) / reference) * 10_000
    : ((reference - clearing) / reference) * 10_000;
}

export function formatImplementationShortfallBps(
  side: "Buy" | "Sell",
  referencePrice: string | undefined,
  clearingPrice: string | undefined,
): string {
  const bps = implementationShortfallBpsValue(side, referencePrice, clearingPrice);
  if (bps === null) return "—";
  const formatted = Math.abs(bps) >= 100 ? bps.toFixed(0) : bps.toFixed(1);
  return `${bps > 0 ? "+" : ""}${formatted} bps`;
}

export function realizedSpreadBpsValue(
  side: "Buy" | "Sell",
  referencePrice: string | undefined,
  clearingPrice: string | undefined,
): number | null {
  const shortfall = implementationShortfallBpsValue(side, referencePrice, clearingPrice);
  return shortfall === null ? null : -shortfall;
}
