...
export function userFacingErrorMessage(
  ...
  if (
    /renewal-backed child|Zylith relay mode requires|renewal package/i.test(
      message
    )
  ) {
    return "This strategy needs renewal enabled. Choose a renewal window and retry.";
  }
  if (/Cancellation witness/i.test(message)) {
    return "Order cancellation is not ready yet. Wait for the latest settlement to sync, then retry.";
  }
  if (
  ...
  if (/\/api\/private\/orders failed with HTTP 400/i.test(message)) {
    return "Private order was rejected by validation. Check available notes and order parameters, then retry.";
  }
  if (
