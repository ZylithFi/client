...
describe("zylithRenewalRelay", () => {
  ...
  it("registers hosted renewal packages without bundling a frontend bearer token", async () => {
    ...
    const { submitZylithRenewalPackage } = await import(
      "./zylithRenewalRelay"
    );

    await submitZylithRenewalPackage(zylithRelayPackage());
  ...
  it("uses package access token headers for hosted results and deletion", async () => {
    ...
    vi.stubGlobal("fetch", fetchMock);
    const { deleteZylithRenewalPackage, fetchZylithRenewalPackageResults } =
      await import("./zylithRenewalRelay");
    const renewalPackage = zylithRelayPackage();
    renewalPackage.access_token = "relay-token";

    await fetchZylithRenewalPackageResults(renewalPackage);
  ...
  it("requires package access token for hosted result access", async () => {
    vi.stubEnv("VITE_ZYLITH_RENEWAL_RELAY_URL", "https://relay.zylith.example");
    const { fetchZylithRenewalPackageResults } = await import(
      "./zylithRenewalRelay"
    );
  ...
  it("normalizes hosted relay abort failures", async () => {
    ...
    await expect(submitZylithRenewalPackage(zylithRelayPackage())).rejects.toThrow(
      "Renewal relay request failed. Check your connection and retry."
    );
  });
  ...
  it("redacts large relay error fields before surfacing failures", async () => {
    ...
    await expect(submitZylithRenewalPackage(zylithRelayPackage())).rejects.toThrow(
      'Renewal relay request failed with HTTP 500: failed for commitment <felt> with "calldata":[...] and "signature":[...] and amount <number>'
    );
  });
});
