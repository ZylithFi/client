...
describe("wallet chain validation", () => {
  ...
  it("accepts matching aliases and rejects mismatched networks", () => {
    ...
    expect(() =>
      ...
      "Wrong Starknet network. Switch to Starknet Sepolia in your wallet and retry."
    );
  });
  ...
  it("reads chain ID from providers exposing the snake_case property", async () => {
    const request = vi.fn(
      async (payload: { type?: string; method?: string }) => {
        if (payload.type === "wallet_signTypedData") return ["0x1", "0x2"];
        return null;
...
describe("wallet-signature sessions", () => {
  ...
  it("switches wallet networks before wallet-signature private session setup", async () => {
    ...
    const request = vi.fn(
      async (payload: { type?: string; params?: { chainId?: string } }) => {
        if (payload.type === "wallet_requestAccounts")
          return [{ address: "0xabc" }];
  ...
  it("deletes corrupt encrypted private stores without retaining copies", async () => {
    ...
    const scope = "acct-test:0x534e5f5345504f4c4941:0x101:0x102:0x102";
  ...
  it("deduplicates concurrent wallet-signature unlock requests", async () => {
    let resolveUnlockSignature: ((signature: string[]) => void) | undefined;
    const signMessage = vi.fn(async () => ["0x1", "0x2"]);
    await selectRuntimeProvider({
      account: {
        address: "0xabc",
    ...
    for (
      let attempt = 0;
      attempt < 20 && signMessage.mock.calls.length === 0;
      attempt += 1
    ) {
  ...
  it("does not create a wallet-signature vault when coordinator vault lookup is offline", async () => {
    ...
    await expect(
      runtime.createWalletWithWalletSignature("0xabc")
    ).rejects.toThrow("Private trading state is unavailable. Retry later.");
  ...
  it("keeps wallet-signature vaults scoped to the connected Starknet account", async () => {
    ...
    await expect(
      runtime.createWalletWithWalletSignature("0xdef")
    ).resolves.toBe(true);
  ...
  it("tries method-style typed-data signing before failing unsupported wallets", async () => {
    const request = vi.fn(
      async (_payload: { type?: string; method?: string }) => {
        throw new Error("unsupported method");
      }
    );
    await selectRuntimeProvider({
    ...
    await expect(
      runtime.createWalletWithWalletSignature("0xabc")
    ).rejects.toThrow("Selected Starknet wallet cannot sign Zylith messages");
    expect(
    ...
    expect(
      request.mock.calls.filter(
        ([payload]) => payload.method === "wallet_signTypedData"
      )
    ).toHaveLength(1);
    expect(
      request.mock.calls.filter(
        ([payload]) => payload.method === "starknet_signTypedData"
      )
    ).toHaveLength(1);
  });
});
...
describe("pending consolidation finalization", () => {
  it("does not mutate local notes before the expected output root is visible on-chain", () => {
    ...
    const result = applyPendingConsolidationRoot(
      records,
      pending,
      "0xdead",
      "scope-a"
...
describe("deposit confirmation polling", () => {
  ...
  it("matches deposit activations from successful transaction calldata", () => {
    ...
    expect(
      transactionCalldataContainsDepositActivation(calldata, {
